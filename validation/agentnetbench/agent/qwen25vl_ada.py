...
# </tool_call>"""
    ...
    def _process_image(self, image_path: str) -> str:
        ...
        return image_b64, resized_width, resized_height
    ...
    def prompt(self, trajectory: Dict[str, Any], current_step: int) -> List[Dict[str, Any]]:
        ...
        for i in range(history_start_idx, current_step):
            ...
            if 'ground_truth_actions' in prev_step:
                ...
                if action_str:
                    self.history_responses.append(f"Step {len(self.history_responses)+1}: {action_str}")

        previous_actions_str = '; '.join(self.history_responses) if self.history_responses else "None"
        ...
        # 仅添加当前 step 图片 + 指令（不论是否有历史，都统一放在一个 user 消息中，保证只有一张图片）
        step = trajectory['steps'][current_step]
        image_file = step.get('image', '')
        if image_file and self.image_dir:

            image_b64, resize_width, resize_height = self._process_image(image_file)
            # messages.append({
            #     "role": "user",
            #     "content": [
            ...
            messages = [
                {
                    "role": "system", 
                    "content": [
                        {"type": "text", "text": WEB_SYS_PROMPT.format(display_width_px=resize_width, display_height_px=resize_height)}
                    ]
                },
                {
                    "role": "user", 
            ...
            # 无图片时仅发送文本指令
            print("No image found for current step or image_dir not set.")
            messages = [
                {
                    "role": "system", 
                    "content": [
                        {"type": "text", "text": WEB_SYS_PROMPT.format(display_width_px=1920, display_height_px=1080)}
                    ]
                },
                {
                    "role": "user",
        ...
        messages.append({"role": "assistant", "content": [{"type": "text", "text": self.prefix}]})
