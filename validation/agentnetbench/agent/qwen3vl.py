...
{previous_actions_str}"""
        ...
        # ------------------------------------------------------------------
        # SINGLE-IMAGE MODE (API限制: 每次请求最多1张图片)
        # 为了避免 "At most 1 image(s) may be provided" 错误，我们只发送当前 step 的截图，
        # 之前的历史步骤全部以文本形式提供（包含低层指令与 tool_call 结果），不再附带图片。
        # ------------------------------------------------------------------
        if len(self.history_responses) > 0:
            for history_idx, history_response in enumerate(self.history_responses):
                step_idx = history_start_idx + history_idx
                h_step = trajectory['steps'][step_idx]
                low_level_instruction = ""
                if 'inner_monologue' in h_step:
                    low_level_instruction = f"Step {step_idx + 1}: {h_step['inner_monologue'].get('low_level_instruction', '')}"
                messages.append({
                    "role": "assistant",
                    "content": [
