...
from agent.qwen25vl_ada import Qwen25VLAda
...
class TrajectoryEvaluator:
    ...
    async def evaluate_trajectory_parallel(self, trajectories: List[Dict[str, Any]], num_cores: int = 4) -> List[Tuple[str, List[Dict[str, Any]]]]:
        ...
        if 'ada' in model_name:
            print(f"Using Qwen25VLAda agent for model: {self.args.model}")
            model = self.args.model.replace('-adaptive', '')
            agent_class = Qwen25VLAda
            agent = agent_class(
                model, 
                client,
                self.args.prefix)

        elif any(qwen_name in model_name for qwen_name in ["qwen2.5-vl", "qwen-vl", "qwen25vl", "qwen2.5vl"]):
...
def main():
    ...
    parser.add_argument("--opencua-image", dest="opencua_image", type=str, default="image_1",
    ...
    parser.add_argument("--prefix", type=str, default="",
                        help="Prefix text for the agent's thought process")
