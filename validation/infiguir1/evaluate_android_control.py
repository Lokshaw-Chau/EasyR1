...
def norm_coordinate(action, width, height):
    ...
    if 'coordinate' in action:
        action['coordinate'] = [action['coordinate'][0]/1000, action['coordinate'][1]/1000]
    if 'coordinate2' in action:
        action['coordinate2'] = [action['coordinate2'][0]/1000, action['coordinate2'][1]/1000]
    return action
    ...
    if current_check_pam['action'] == 'wait':
        ...
        if pred_action['action'] == 'open':
            ...
            if len(current_check_pam.get('candidate_bbox', []))>0:
                return True, check_click(pred_action['coordinate'], current_check_pam['candidate_bbox'], gt_point=[(current_check_pam['candidate_bbox'][0][0]+current_check_pam['candidate_bbox'][0][2])/2, (current_check_pam['candidate_bbox'][0][1]+current_check_pam['candidate_bbox'][0][3])/2], width=width, height=height)
            else:
                return False, False
        else:
            return False, False
        ...
        if pred_action['action'] == current_check_pam['action']:
            return True, check_click(pred_action['coordinate'], current_check_pam['candidate_bbox'], gt_point=current_check_pam['coordinate'], width=width, height=height)
        else:
            return False, False
