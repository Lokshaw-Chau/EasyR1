...
class AndroidControl:
    ...
    def generate_jobs(self, total_split, split) -> List[Dict]:
        ...
        for line in tqdm(lines, desc='Generating jobs'):
            ...
            h_bar, w_bar = smart_resize(height, width, max_pixels=self.max_pixels, factor=28)
            ...
            for i in range(len(line["step_check_pams"])):
                ...
                task_progress.append(line["episode"]["step_instructions"][i])
                jobs.append({
    ...
    def compute_scores(self, jobs) -> Dict[str, Any]:
        ...
        for job in tqdm(jobs, desc='Computing scores'):
            try:
                ...
                pred = output
                pred = re.search(r"<tool_call>(.*?)</tool_call>", pred, re.DOTALL)
                pred = pred.group(1)
