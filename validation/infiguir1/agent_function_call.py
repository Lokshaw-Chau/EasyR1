...
@register_tool("old_mobile_use")
class OldMobileUse(BaseTool):
    @property
    def description(self):
        return f"""
Use a touchscreen to interact with a mobile device, and take screenshots.
...
@register_tool("gui_action")
class GUIAction(BaseTool):
    @property
    def description(self):
        return f"""
Click on the screen to perform the user command.
* The screen's resolution is {self.display_width_px}x{self.display_height_px}.
* Make sure to click any buttons, links, icons, etc with the cursor tip in the center of the element. Don't click boxes on their edges unless asked.
""".strip()
