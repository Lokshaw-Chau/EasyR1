...
def evaluate(args):
    ...
    for pred, gt in zip(prediction, ground_truth):
        ...
        #     category=gt['group']+'-'+gt['gt_action']+'-'+'swipe'
        #     gt_text=gt['gt_input_text']
        #     pred_text=pred['pred_input_text']
        #     score_dict[category+"_"+"full"] += 1
        #     #if calculate_f1_score(gt_text,pred_text)>=0.5:
        #     score_dict[category] += 1
        # if gt['gt_action'] in ['system_button']:
        #     category=gt['group']+'-'+gt['gt_action']+'-'+'system_button'
        #     gt_text=gt['gt_input_text']
        #     pred_text=pred['pred_input_text']
        #     score_dict[category+"_"+"full"] += 1
        #     if calculate_f1_score(gt_text,pred_text)>=0.5:
        #         score_dict[category] += 1
        # if gt['gt_action'] in ['terminate']:
        #     category=gt['group']+'-'+gt['gt_action']+'-'+'terminate'
        #     gt_text=gt['gt_input_text']
        #     pred_text=pred['pred_input_text']
        #     score_dict[category+"_"+"full"] += 1
        #     if calculate_f1_score(gt_text,pred_text)>=0.5:
    ...
    logger.info(f"ALL Type : {(full_type_hit / full_type) * 100}")
    logger.info(f"ALL Step : {(full_step_hit / full_step)* 100}")
    logger.info(f"ALL GR : {(full_gr_hit / full_gr)* 100}")
    logger.info(f"ALL Think : {think_cnt/len(prediction)* 100}")
