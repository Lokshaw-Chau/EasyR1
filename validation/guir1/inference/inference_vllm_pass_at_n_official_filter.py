...
def main(args):
    ...
    if args.n != 1:
        ...
        for item in old_file_data:
            think_pass_list = item['<think>_pass']
            tool_call_pass_list = item['<tool_call>_pass']
            if any(think_pass_list) and any(tool_call_pass_list):
                continue
            new_data_id_list.append(item['instruction']+str(item['gt_bbox']))

        # if id not in new_data_id_list, drop the sample
        data = [item for item in data if (item['instruction']+str(item['gt_bbox'])) in new_data_id_list]
